...
class gallery_task_Core
{
    ...
    public static function available_tasks()
    {
        ...
        $tasks       = [];
        $tasks[]     = Task_Definition::factory()->callback('gallery_task::rebuild_dirty_images')->name(t('Rebuild Images'))->description($dirty_count ? t2(
            'You have one out of date photo',
            'You have %count out of date photos',
                                                                                                                                                            $dirty_count
        ) : t('All your photos are up to date'))->severity($dirty_count ? log::WARNING : log::SUCCESS);
    ...
    public static function rebuild_dirty_images($task)
    {
        ...
        try {
            ...
            foreach ($result as $row) {
                ...
                if ($item->loaded()) {
                    try {
                        ...
                        $errors[] = t("Successfully rebuilt images for '%title'", ['title' => html::purify($item->title)]);
                    } catch (Exception $e) {
                        $errors[]           = t("Unable to rebuild images for '%title'", ['title' => html::purify($item->title)]);
                        $errors[]           = (string)$e;
                        $ignored[$item->id] = 1;
                    }
                }
            ...
            $task->status = t2('Updated: 1 image. Total: %total_count.', 'Updated: %count images. Total: %total_count.', $completed, ['total_count' => $total_count]);
    ...
    public static function update_l10n($task)
    {
        ...
        try {
            ...
            if (!$task->done) {
                Cache::instance()->set("update_l10n_cache:{$task->id}", serialize([$dirs, $files, $cache, $num_fetched]), ['l10n']);
            } else {
        ...
        try {
            ...
            switch ($task->get('mode', 'init')) {
                case 'init':
                    ...
                    Cache::instance()->set("file_cleanup_cache:{$task->id}", serialize($files), ['file_cleanup']);
                    if (0 == count($files)) {
                        break;
                    }
            ...
            $task->status = t2('Removed: 1 file. Total: %total_count.', 'Removed: %count files. Total: %total_count.', $current, ['total_count' => $total]);
    ...
    public static function fix($task)
    {
        $start = microtime(true);
                ...
                case self::FIX_STATE_RUN_MPTT:
                    ...
                    switch ($ptr_mode) {
                        case 'L':
                            ...
                            $level++;
                            foreach (db::build()->select(['id', 'type'])->from('items')->where('parent_id', '=', $id)->order_by('left_ptr', 'DESC')// DESC since array_pop effectively reverses them
                                       ->execute() as $child) {
        ...
        $task->status = t2('One operation complete', '%count / %total operations complete', $completed, ['total' => $total]);
    }

    public static function find_dupe_slugs()
    {
        return db::build()->select_distinct(['parent_slug' => db::expr("CONCAT(`parent_id`, ':', LOWER(`slug`))")])->select('id')->select(['C' => 'COUNT("*")'])->from('items')->having('COUNT("*")', '>', 1)->group_by('parent_slug', 'id')->execute();
    }

    public static function find_dupe_names()
    {
        // looking for photos, movies, and albums
        return db::build()->select_distinct(['parent_name' => db::expr("CONCAT(`parent_id`, ':', LOWER(`name`))")])->select('id')->select(['C' => 'COUNT("*")'])->from('items')->having('COUNT("*")', '>', 1)->group_by('parent_name', 'id')->execute();
    }

    public static function find_dupe_base_names()
    {
        // looking for photos or movies, not albums
        return db::build()->select_distinct(['parent_base_name' => db::expr("CONCAT(`parent_id`, ':', LOWER(SUBSTR(`name`, 1, LOCATE('.', `name`) - 1)))")])->select('id')->select(['C' => 'COUNT("*")'])->from('items')->where('type', '<>', 'album')->having(
            'COUNT("*")',
            '>',
                                                                                                                                                                                                                                                               1
        )->group_by(
    ...
    public static function find_empty_item_caches($limit)
    {
        return db::build()->select('items.id')->from('items')->where('relative_path_cache', 'is', null)->or_where('relative_url_cache', 'is', null)->limit($limit)->execute();
    }

    public static function find_missing_access_caches()
    {
        return self::find_missing_access_caches_limited(1 << 16);
    }

    public static function find_missing_access_caches_limited($limit)
    {
        return db::build()->select('items.id')->from('items')->join('access_caches', 'items.id', 'access_caches.item_id', 'left')->where('access_caches.id', 'is', null)->limit($limit)->execute();
    }
}
