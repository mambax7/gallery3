...
class gallery_task_Core
{
    ...
    public static function rebuild_dirty_images($task)
    {
        ...
        ->execute();
            ...
            $task->status = t2(
          "Updated: 1 image. Total: %total_count.",
    ...
    public static function update_l10n($task)
    {
      ...
      case "find_files":  // 0% - 10%
        ...
        $task->status = t2(
            "Finding files: found 1 file",
                           "Finding files: found %count files",
            count($files)
        );
      ...
      case "scan_files": // 10% - 70%
        ...
        $task->status = t2(
            "Scanning files: scanned 1 file",
                           "Scanning files: scanned %count files",
    ...
    public static function file_cleanup($task)
    {
      ...
      case "delete_files":
        ...
        $task->set("current", $current);
            ...
            $task->status = t2(
          "Removed: 1 file. Total: %total_count.",
    ...
    public static function fix($task)
    {
      ...
      case self::FIX_STATE_RUN_MISSING_ACCESS_CACHES:
        ...
        "%count / %total operations complete",
        $completed,
                       array("total" => $total)
    );
    }
