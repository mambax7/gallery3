...
class gallery_task_Core
{
    ...
    public static function available_tasks()
    {
        ...
        $tasks[] = Task_Definition::factory()
                 ...
                 ->description($dirty_count ?
                               t2(
                                   'You have one out of date photo', 'You have %count out of date photos',
                                   $dirty_count
                               )
    ...
    public static function update_l10n($task)
    {
      ...
      case 'find_files':  // 0% - 10%
        ...
        $task->status = t2(
            'Finding files: found 1 file', 'Finding files: found %count files',
            count($files)
        );
      ...
      case 'scan_files': // 10% - 70%
        ...
        $task->status = t2(
            'Scanning files: scanned 1 file', 'Scanning files: scanned %count files',
