...
class graphics_Core
{
    ...
    public static function add_rule($module_name, $target, $operation, $args, $priority)
    {
        ...
        graphics::mark_dirty('thumb' == $target, 'resize' == $target);
    }
     ...
     * @param string $operation   the name of the operation(<defining class>::method)
     */
    public static function remove_rule($module_name, $target, $operation)
    {
        db::build()->delete('graphics_rules')->where('module_name', '=', $module_name)->where('target', '=', $target)->where('operation', '=', $operation)->execute();
     ...
     */
    public static function activate_rules($module_name)
    {
        db::build()->update('graphics_rules')->set('active', true)->where('module_name', '=', $module_name)->execute();
    }
     ...
     * module it won't cause all of your images to suddenly require a rebuild.
     */
    public static function deactivate_rules($module_name)
    {
        db::build()->update('graphics_rules')->set('active', false)->where('module_name', '=', $module_name)->execute();
    }
    ...
    private static function _replace_image_with_placeholder($item, $target)
    {
        ...
        } elseif ($item->is_movie() || ($item->is_album() && $item->album_cover()->is_movie())) {
    ...
    private static function _get_rules($target)
    {
        if (empty(self::$_rules_cache[$target])) {
            $rules = [];
            foreach (ORM::factory('graphics_rule')->where('target', '=', $target)->where('active', '=', true)->order_by('priority', 'asc')->find_all() as $rule) {
    ...
    {
        return db::build()->from('items')->and_open()->where('thumb_dirty', '=', 1)->and_open()->where('type', '<>', 'album')->or_where('album_cover_item_id', 'IS NOT', null)->close()->or_open()->where('resize_dirty', '=', 1)->where('type', '=', 'photo')->close()->close();
    }
     ...
     */
    public static function mark_dirty($thumbs, $resizes, $type = null, $mime_type = null)
    {
        if ($thumbs || $resizes) {
        ...
        if ($count) {
            site_status::warning(t2('One of your photos is out of date. <a %attrs>Click here to fix it</a>', '%count of your photos are out of date. <a %attrs>Click here to fix them</a>', $count, [
                                                                                                               'attrs' => html::mark_clean(sprintf('href="%s" class="g-dialog-link"', url::site('admin/maintenance/start/gallery_task::rebuild_dirty_images?csrf=__CSRF__')))
                                                                                                           ]), 'graphics_dirty');
        }
    }
    ...
    public static function detect_toolkits()
    {
        ...
        if (!isset($gd['GD Version'])) {
            ...
            if (!$toolkits->gd->rotate && !$toolkits->gd->sharpen) {
                $toolkits->gd->error = t('You have GD version %version, but it lacks image rotation and sharpening.', ['version' => $gd['GD Version']]);
            } elseif (!$toolkits->gd->rotate) {
                $toolkits->gd->error = t('You have GD version %version, but it lacks image rotation.', ['version' => $gd['GD Version']]);
            } elseif (!$toolkits->gd->sharpen) {
                $toolkits->gd->error = t('You have GD version %version, but it lacks image sharpening.', ['version' => $gd['GD Version']]);
            }
        }
        ...
        if (!function_exists('exec')) {
            ...
            foreach ($magick_kits as $index => $settings) {
                ...
                if ($path) {
                    if (@is_file($path)
                        && preg_match($settings['version_regex'], shell_exec($path . ' ' . $settings['version_arg']), $matches)) {
                        $version = $matches[1];
                        ...
                        $toolkits->$index->error     = t("%toolkit_name is installed, but PHP's open_basedir restriction prevents Gallery from using it.", ['toolkit_name' => $settings['name']]);
                    }
                } else {
                    $toolkits->$index->installed = false;
                    $toolkits->$index->error     = t('We could not locate %toolkit_name on your system.', ['toolkit_name' => $settings['name']]);
                }
            }
        }
     ...
     */
    public static function choose_default_toolkit()
    {
        ...
        if (!module::get_var('gallery', 'graphics_toolkit')) {
            site_status::warning(t('Graphics toolkit missing!  Please <a href="%url">choose a toolkit</a>', ['url' => html::mark_clean(url::site('admin/graphics'))]), 'missing_graphics_toolkit');
        }
    }
    ...
    public static function init_toolkit()
    {
        ...
        switch (module::get_var('gallery', 'graphics_toolkit')) {
            ...
            case 'imagemagick':
                ...
                Kohana_Config::instance()->set('image.params.directory', module::get_var('gallery', 'graphics_toolkit_path'));
                break;
     ...
     * @return boolean
     */
    public static function can($func)
    {
        if ('gd' == module::get_var('gallery', 'graphics_toolkit')) {
            switch ($func) {
                case 'rotate':
