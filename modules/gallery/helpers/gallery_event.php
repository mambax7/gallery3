...
class gallery_event_Core
{
    ...
    public static function gallery_shutdown()
    {
    ...
    public static function user_deleted($user)
    {
        ...
        if (!empty($admin)) {          // could be empty if there is not identity provider
            db::build()->update('tasks')->set('owner_id', $admin->id)->where('owner_id', '=', $user->id)->execute();
            db::build()->update('items')->set('owner_id', $admin->id)->where('owner_id', '=', $user->id)->execute();
            db::build()->update('logs')->set('user_id', $admin->id)->where('user_id', '=', $user->id)->execute();
        }
    }
    ...
    public static function group_deleted($group)
    {
    ...
    public static function item_created($item)
    {
        ...
        try {
            ...
            log::error('graphics', t("Couldn't create a thumbnail or resize for %item_title", ['item_title' => $item->title]), html::anchor($item->abs_url(), t('details')));
    ...
    public static function batch_complete()
    {
    ...
    public static function item_moved($item, $old_parent)
    {
        if ($item->is_album()) {
    ...
    public static function user_auth_failed($name)
    {
        auth::record_failed_attempt($name);
    }

    public static function user_auth($user)
    {
        auth::clear_failed_attempts($user);
        Session::instance()->set('active_auth_timestamp', time());
    }
            ...
            if (!empty($item)) {
                ...
                if ($can_add) {
                    ...
                    if ($is_album_writable) {
                        ...
                        message::warning(t("The album '%album_name' is not writable.", ['album_name' => $item->title]));
                    }
                }
                ...
                if ($can_edit && $item->is_photo() && graphics::can('rotate')) {
                    $options_menu->append(Menu::factory('ajax_link')->id('rotate_ccw')->label(t('Rotate 90° counter clockwise'))->css_class('ui-icon-rotate-ccw')->ajax_handler('function(data) { '
                                                                                                                                                                                . "\$.gallery_replace_image(data, \$('$item_css_selector')) }")->url(url::site("quick/rotate/$item->id/ccw?csrf=$csrf&amp;from_id={$item->id}&amp;page_type=$page_type")))->append(Menu::factory('ajax_link')->id('rotate_cw')->label(t('Rotate 90° clockwise'))->css_class('ui-icon-rotate-cw')->ajax_handler('function(data) { '
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               . "\$.gallery_replace_image(data, \$('$item_css_selector')) }")->url(url::site("quick/rotate/$item->id/cw?csrf=$csrf&amp;from_id={$item->id}&amp;page_type=$page_type")));
                }
    ...
    public static function context_menu($menu, $theme, $item, $thumb_css_selector)
    {
        ...
        if (access::can('edit', $item)) {
            ...
            $options_menu->append(Menu::factory('dialog')->id('edit')->label($edit_title)->css_class('ui-icon-pencil')->url(url::site("quick/form_edit/$item->id?from_id={$theme_item->id}")));

            if ($item->is_photo() && graphics::can('rotate')) {
                $options_menu->append(Menu::factory('ajax_link')->id('rotate_ccw')->label(t('Rotate 90° counter clockwise'))->css_class('ui-icon-rotate-ccw')->ajax_handler('function(data) { '
                                                                                                                                                                            . "\$.gallery_replace_image(data, \$('$thumb_css_selector')) }")->url(url::site("quick/rotate/$item->id/ccw?csrf=$csrf&amp;from_id={$theme_item->id}&amp;page_type=$page_type")))->append(Menu::factory('ajax_link')->id('rotate_cw')->label(t('Rotate 90° clockwise'))->css_class('ui-icon-rotate-cw')->ajax_handler('function(data) { '
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  . "\$.gallery_replace_image(data, \$('$thumb_css_selector')) }")->url(url::site("quick/rotate/$item->id/cw?csrf=$csrf&amp;from_id={$theme_item->id}&amp;page_type=$page_type")));
            }
            ...
            if (access::can('edit', $parent)) {
                ...
                if (1 != $item->parent()->id) {
                    $options_menu->append(Menu::factory('ajax_link')->id('make_album_cover')->label($cover_title)->css_class("ui-icon-star $disabledState")->ajax_handler('function(data) { window.location.reload() }')->url(url::site("quick/make_album_cover/$item->id?csrf=$csrf")));
                }
                $options_menu->append(Menu::factory('dialog')->id('delete')->label($delete_title)->css_class('ui-icon-trash')->url(url::site("quick/form_delete/$item->id?csrf=$csrf&amp;" . "from_id={$theme_item->id}&amp;page_type=$page_type")));
            }

            if ($item->is_album()) {
                $options_menu->append(Menu::factory('dialog')->id('add_item')->label(t('Add a photo'))->css_class('ui-icon-plus')->url(url::site("uploader/index/$item->id")))->append(Menu::factory('dialog')->id('add_album')->label(t('Add an album'))->css_class('ui-icon-note')->url(url::site("form/add/albums/$item->id?type=album")))->append(Menu::factory('dialog')->id('edit_permissions')->label(t('Edit permissions'))->css_class('ui-icon-key')->url(url::site("permissions/browse/$item->id")));
            }
        }
    }
