...
class movie_Core
{
     ...
     */
    public static function allow_uploads()
    {
     ...
     * date can be useful.
     */
    public static function get_ffmpeg_version()
    {
        $ffmpeg = movie::find_ffmpeg();
        if (empty($ffmpeg)) {
            return array('', '');
    ...
    public static function get_file_metadata($file_path)
    {
        ...
        if (!empty($ffmpeg)) {
            ...
            } elseif (preg_match("/duration.*?:.*?(\d+)/", $result, $matches)) {
     ...
     */
    public static function hhmmssdd_to_seconds($hhmmssdd)
    {
