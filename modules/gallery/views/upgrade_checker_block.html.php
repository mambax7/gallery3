...
<p>
    ...
    <?php else: ?>
        <?= t(
    "You are using an experimental snapshot of Gallery %version (branch %branch) but your gallery3/.build_number file is missing so we don't know what build you have.  You should probably upgrade so that you have that file.",
              ['version' => gallery::VERSION, 'branch' => gallery::RELEASE_BRANCH, 'build_number' => $build_number]
) ?>
    <?php endif ?>
</p>
...
<p class="g-text-small">
    ...
    <?php else: ?>
        <?= t('The last upgrade check was made on %date.', ['date' => gallery::date_time($version_info->timestamp)]) ?>
    <?php endif ?>
</p>
