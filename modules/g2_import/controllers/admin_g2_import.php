...
class Admin_g2_import_Controller extends Admin_Controller
{
    ...
    private function _get_import_form()
    {
        ...
        $group->embed_path->error_messages(
            'invalid',
            t('The path you entered is not a Gallery 2 installation.')
    );
        $group->embed_path->error_messages(
            'broken',
            t("Your Gallery 2 install isn't working properly.  Please verify it!")
    );
        $group->embed_path->error_messages(
            'missing',
            t('The path you entered does not exist.')
    );
