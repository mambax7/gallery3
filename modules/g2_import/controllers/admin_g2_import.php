...
class Admin_g2_import_Controller extends Admin_Controller
{
    public function index()
    {
        ...
        if (g2_import::is_initialized()) {
            if ((bool)ini_get('eaccelerator.enable') || (bool)ini_get('xcache.cacher')) {
                message::warning(t(
                    "The eAccelerator and XCache PHP performance extensions are known to cause issues.  If you're using either of those and are having problems, please disable them while you do your import.  Add the following lines: <pre>%lines</pre> to gallery3/.htaccess and remove them when the import is done.",
            ...
            foreach (['notification', 'search', 'exif'] as $module_id) {
                if (module::is_active($module_id)) {
                    message::warning(t('<a href="%url">Deactivating</a> the <b>%module_id</b> module during your import will make it faster', ['url' => url::site('admin/modules'), 'module_id' => $module_id]));
                }
            }
            if (module::is_active('akismet')) {
                message::warning(t('The Akismet module may mark some or all of your imported comments as spam.  <a href="%url">Deactivate</a> it to avoid that outcome.', ['url' => url::site('admin/modules')]));
            }
        } elseif (g2_import::is_configured()) {
    ...
    public function save()
    {
        ...
        if ($form->validate()) {
            ...
            if ('ok' == ($g2_init_error = g2_import::is_valid_embed_path($embed_path))) {
    ...
    private function _get_import_form()
    {
        ...
        $form       = new Forge('admin/g2_import/save', '', 'post', ['id' => 'g-admin-configure-g2-import-form']);
        $group      = $form->group('configure_g2_import')->label(t('Configure Gallery 2 Import'));
        $group->input('embed_path')->label(t('Filesystem path to your Gallery 2 embed.php file'))->value($embed_path);
        $group->embed_path->error_messages('invalid', t('The path you entered is not a Gallery 2 installation.'));
        $group->embed_path->error_messages('broken', t("Your Gallery 2 install isn't working properly.  Please verify it!"));
        $group->embed_path->error_messages('missing', t('The path you entered does not exist.'));
