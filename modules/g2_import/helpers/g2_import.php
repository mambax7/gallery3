...
class g2_import_Core
{
    ...
    public static function is_valid_embed_path($embed_path)
    {
     ...
     */
    public static function init_embed($embed_path)
    {
        ...
        try {
            ...
            if (!file_exists($mod_path) || !file_exists("$mod_path/embed.php")) {
                ...
                file_put_contents("$mod_path/embed.php", str_replace([
                                                                     ...
                                                                     ], array_merge(["<?php defined(\"SYSPATH\") || die(\"No direct script access.\") ?>\n"], file("$base_dir/embed.php"))));

                file_put_contents("$mod_path/main.php", str_replace([
                ...
                file_put_contents("$mod_path/bootstrap.inc", str_replace([
                ...
                file_put_contents("$mod_path/Gallery.class", str_replace([
                                                                             'class Gallery',
                                                                             'function Gallery'
                                                                         ], [
                                                                             'class G2_Gallery',
     ...
     */
    public static function g2_module_active($module)
    {
        static $plugin_list;
        if (!$plugin_list) {
     ...
     */
    public static function g2_stats()
    {
        global $gallery;
    ...
    public static function g3_stats()
    {
        ...
        foreach (db::build()->select('resource_type')->select(['C' => 'COUNT("*")'])->from('g2_maps')->where('resource_type', 'IN', ['album', 'comment', 'item', 'user', 'group'])->group_by('resource_type')->execute() as $row) {
    ...
    public static function import_group(&$queue)
    {
        ...
        try {
            ...
            throw new G2_Import_Exception(t('Failed to import Gallery 2 group with id: %id,', ['id' => $g2_group_id]), $e);
        }
        ...
        switch ($g2_group->getGroupType()) {
            case GROUP_NORMAL:
                try {
                    ...
                    $messages[] = t("Group '%name' was imported", ['name' => $g2_group->getGroupname()]);
                } catch (Exception $e) {
                    // Did it fail because of a duplicate group name?
                    $group = identity::lookup_group_by_name($g2_group->getGroupname());
                    if ($group) {
                        $messages[] = t("Group '%name' was mapped to the existing group group of the same name.", ['name' => $g2_group->getGroupname()]);
                    } else {
                        throw new G2_Import_Exception(t("Failed to import group '%name'", ['name' => $g2_group->getGroupname()]), $e);
                    }
                }
    ...
    public static function import_user(&$queue)
    {
        ...
        if (self::map($g2_user_id)) {
            return t('User with id: %id already imported, skipping', ['id' => $g2_user_id]);
        }
        ...
        if ($user) {
            ...
            try {
                $user = identity::create_user($g2_user->getUserName(), $g2_user->getFullName(), // Note: The API expects a password in cleartext.
                ...
                throw new G2_Import_Exception(t("Failed to create user: '%name' (id: %id)", ['name' => $g2_user->getUserName(), 'id' => $g2_user_id]), $e, $messages);
            }
            ...
            if ('unknown@unknown.com' == $email) {
                $messages[] = t("Fixed invalid email (was '%invalid_email')", ['invalid_email' => $g2_user->getEmail()]);
            }
        }
        ...
        try {
            ...
            throw new G2_Import_Exception(t("Failed to create user: '%name'", ['name' => $user->name]), $e, $messages);
        }
     ...
     */
    public static function import_album(&$queue)
    {
        ...
        if (null == $g2_album->getParentId()) {
            ...
            try {
                ...
                throw new G2_Import_Exception(t('Failed to import Gallery 2 album with id %id and name %name.', ['id' => $g2_album_id, 'name' => $album->name]), $e);
            }

            self::import_keywords_as_tags($g2_album->getKeywords(), $album);
        }

        self::set_map($g2_album_id, $album->id, 'album', self::g2_url(['view' => 'core.ShowItem', 'itemId' => $g2_album->getId()]));
     ...
     */
    public static function set_album_values($album, $g2_album)
    {
     ...
     */
    public static function set_album_highlight(&$queue)
    {
        // Dequeue the current album and enqueue its children
        list($g2_album_id, $children) = each($queue);
        unset($queue[$g2_album_id]);
        ...
        if (isset($table[$g2_album_id])) {
            ...
            if ($item_id) {
                ...
                try {
                    ...
                    return (string)new G2_Import_Exception(t("Failed to generate an album highlight for album '%name'.", ['name' => $g3_album->name]), $e);
                }

                self::set_map($orig_g2_source->getId(), $g3_album->id, 'thumbnail', self::g2_url(['view' => 'core.DownloadItem', 'itemId' => $orig_g2_source->getId()]));
            }
        }
    }
     ...
     */
    public static function import_item(&$queue)
    {
        ...
        if (!file_exists($g2_path)) {
            ...
            g2_import::log(t('%path missing in import; replacing it with a placeholder', ['path' => $g2_path]));
        ...
        switch ($g2_type) {
            case 'GalleryPhotoItem':
                if (!in_array($g2_item->getMimeType(), ['image/jpeg', 'image/gif', 'image/png'])) {
                    ...
                    $messages[] = t("'%path' is an unsupported image type, using a placeholder", ['path' => $g2_path]);
                    $g2_path    = MODPATH . 'g2_import/data/broken-image.gif';
                    $corrupt    = 1;
                }
                try {
                    ...
                    $exception_info = (string)new G2_Import_Exception(t("Corrupt image '%path'", ['path' => $g2_path]), $e, $messages);
            ...
            case 'GalleryMovieItem':
                ...
                if (in_array($g2_item->getMimeType(), ['video/mp4', 'video/x-flv'])) {
                    try {
                        ...
                        $exception_info = (string)new G2_Import_Exception(t("Corrupt movie '%path'", ['path' => $g2_path]), $e, $messages);
        ...
        if (isset($item)) {
            ...
            self::set_map($g2_item_id, $item->id, 'file', self::g2_url(['view' => 'core.DownloadItem', 'itemId' => $g2_item_id]));
            ...
            if (!empty($derivatives[$g2_item_id])) {
                foreach ($derivatives[$g2_item_id] as $derivative) {
                    ...
                    self::set_map($derivative->getId(), $item->id, $resource_type, self::g2_url(['view' => 'core.DownloadItem', 'itemId' => $derivative->getId()]));
                }
            }
        }
        ...
        if ($corrupt) {
            if (!empty($item)) {
                ...
                $messages[] = t('<a href="%g2_url">%title</a> from Gallery 2 could not be processed; (imported as <a href="%g3_url">%title</a>)', [
                                                                                                                                                    'g2_url' => $g2_item_url,
                                                                                                                                                    'g3_url' => $item->url(),
                                                                                                                                                    'title'  => $title
                                                                                                                                                ]);
            } else {
                $messages[] = t('<a href="%g2_url">%title</a> from Gallery 2 could not be processed', ['g2_url' => $g2_item_url, 'title' => $g2_item->getTitle()]);
            }
        }
    ...
    private static function _import_permissions($g2_album, $g3_album)
    {
        ...
        if (null != $g2_album->getParentId()
            && g2(GalleryCoreApi::fetchAccessListId($g2_album->getId())) == g2(GalleryCoreApi::fetchAccessListId($g2_album->getParentId()))) {
            return;
        }
        ...
        foreach ($granted_permissions as $group_id => $permissions) {
            ...
            } elseif ($permissions != $granted_parent_permissions[$group_id]) {
     ...
     */
    public static function import_comment(&$queue)
    {
        ...
        try {
            ...
            return t("Failed to load Gallery 2 comment with id: %id\%exception", ['id' => $g2_comment_id, 'exception' => (string)$e]);
        }
        ...
        try {
            ...
            return (string)new G2_Import_Exception(t('Failed to import comment with id: %id.', ['id' => $g2_comment_id]), $e);
        }
     ...
     */
    public static function import_tags_for_item(&$queue)
    {
    ...
    public static function import_keywords_as_tags($keywords, $item)
    {
     ...
     */
    public static function copy_matching_thumbnails_and_resizes($item)
    {
        ...
        try {
            ...
            return (string)new G2_Import_Exception(t("Failed to copy thumbnails and resizes for item '%name' (Gallery 2 id: %id)", ['name' => $item->name, 'id' => $g2_item_id]), $e);
        }
    }
    ...
    public static function common_sizes()
    {
        ...
        foreach ([
            ...
            $results      = g2($gallery->search('SELECT COUNT(*) AS c, [GalleryDerivativeImage::width] '
                                                ...
                                                . 'ORDER by c DESC', [$g2_enum], ['limit' => [1]]));
            $row          = $results->nextResult();
            $sizes[$type] = ['size' => $row[1], 'count' => $row[0]];

            $results = g2($gallery->search('SELECT COUNT(*) AS c, [GalleryDerivativeImage::height] '
                                           . 'FROM [GalleryDerivativeImage], [GalleryDerivative] '
                                           . 'WHERE [GalleryDerivativeImage::id] = [GalleryDerivative::id] '
                                           . '  AND [GalleryDerivative::derivativeType] = ? '
            ...
            $results               = g2($gallery->search('SELECT COUNT(*) FROM [GalleryDerivative] WHERE [GalleryDerivative::derivativeType] = ?', [$g2_enum]));
            $row                   = $results->nextResult();
     ...
     */
    public static function extract_description($g2_item)
    {
    ...
    private static function _transform_bbcode($text)
    {
        if (false !== strpos($text, '[')) {
            $text = preg_replace(array_keys(self::$bbcode_mappings), array_values(self::$bbcode_mappings), $text);
        }
        return $text;
    }
     ...
     */
    public static function get_item_ids($min_id)
    {
        global $gallery;

        $ids     = [];
        $results = g2($gallery->search('SELECT [GalleryItem::id] '
                                       ...
                                       . 'AND [GalleryItem::id] > ? '
                                       . 'ORDER BY [GalleryItem::id] ASC', [$min_id], ['limit' => ['count' => 100]]));
     ...
     */
    public static function get_comment_ids($min_id)
    {
        global $gallery;

        $ids     = [];
        $results = g2($gallery->search('SELECT [GalleryComment::id] ' . 'FROM [GalleryComment] ' . 'WHERE [GalleryComment::publishStatus] = 0 ' . // 0 == COMMENT_PUBLISH_STATUS_PUBLISHED
                                       'AND [GalleryComment::id] > ? ' . 'ORDER BY [GalleryComment::id] ASC', [$min_id], ['limit' => ['count' => 100]]));
        while ($result = $results->nextResult()) {
            $ids[] = $result[0];
        }
        return $ids;
     ...
     * next chunk of comments to import.
     */
    public static function get_tag_item_ids($min_id)
    {
        global $gallery;

        $ids     = [];
        $results = g2($gallery->search('SELECT DISTINCT([TagItemMap::itemId]) FROM [TagItemMap] ' . 'WHERE [TagItemMap::itemId] > ? ' . 'ORDER BY [TagItemMap::itemId] ASC', [$min_id], ['limit' => ['count' => 100]]));
        while ($result = $results->nextResult()) {
            $ids[] = $result[0];
        }
        return $ids;
     ...
     */
    public static function get_user_ids($min_id)
    {
        global $gallery;

        $ids     = [];
        $results = g2($gallery->search('SELECT [GalleryUser::id] ' . 'FROM [GalleryUser] ' . 'WHERE [GalleryUser::id] > ? ' . 'ORDER BY [GalleryUser::id] ASC', [$min_id], ['limit' => ['count' => 100]]));
        while ($result = $results->nextResult()) {
            $ids[] = $result[0];
        }
        return $ids;
     ...
     */
    public static function get_group_ids($min_id)
    {
        global $gallery;

        $ids     = [];
        $results = g2($gallery->search('SELECT [GalleryGroup::id] ' . 'FROM [GalleryGroup] ' . 'WHERE [GalleryGroup::id] > ? ' . 'ORDER BY [GalleryGroup::id] ASC', [$min_id], ['limit' => ['count' => 100]]));
        while ($result = $results->nextResult()) {
            $ids[] = $result[0];
        }
        return $ids;
     ...
     */
    public static function map($g2_id)
    {
     ...
     */
    public static function set_map($g2_id, $g3_id, $resource_type, $g2_url = null)
    {
     ...
     */
    public static function clear_map($g2_id, $resource_type)
    {
        db::build()->delete('g2_maps')->where('g2_id', '=', $g2_id)->where('resource_type', '=', $resource_type)->execute();
    }
    ...
    public static function g2_url($params)
    {
        global $gallery;
        return $gallery->getUrlGenerator()->generateUrl($params, [
                                                                   'forceSessionId' => false,
                                                                   'htmlEntities'   => false,
                                                                   'urlEncode'      => false,
                                                                   'useAuthToken'   => false
    ...
    public static function lower_error_reporting()
    {
