...
class ORM_Core
{
    ...
    public function __construct($id = null)
    {
        ...
        if (!isset($this->sorting)) {
            // Default sorting
            $this->sorting = [$this->primary_key => 'asc'];
        }
    ...
    public function __sleep()
    {
        ...
        return ['object_name', 'object', 'changed', '_loaded', '_saved', 'sorting'];
    }
    ...
    public function __call($method, array $args)
    {
        if (method_exists($this->db_builder, $method)) {
            if (in_array($method, ['execute', 'insert', 'update', 'delete'])) {
            ...
            if ('select' === $method and $num_args > 3) {
                ...
                switch ($num_args) {
                    case 0:
                        if (in_array($method, ['open', 'and_open', 'or_open', 'close', 'cache'])) {
                    ...
                    default:
                        ...
                        call_user_func_array([$this->db, $method], $args);
                        break;
                }
            }
            ...
            throw new Kohana_Exception('Invalid method :method called in :class', [':method' => $method, ':class' => get_class($this)]);
        }
    }
    ...
    public function __get($column)
    {
        if (array_key_exists($column, $this->object)) {
            ...
            if (in_array($model->object_name, $this->belongs_to)) {
                ...
                $where = [$model->foreign_key(true), '=', $this->object[$this->foreign_key($column)]];
            } else {
            ...
            return $model->join($join_table, $join_col1, $join_col2)->where($through->foreign_key($this->object_name, $join_table), '=', $this->primary_key_value);
    ...
    public function with($target_path)
    {
        ...
        $join_table = [$this->db->quote_table($target_path) => $target->db->quote_table($target->table_name)];
                ...
                foreach ($parameters as $type => $value) {
                    switch ($type) {
                        case 'pre_filter':
                            ...
                            break;
                        case 'rules':
                            $rules = array_merge([$field], $value);
                            call_user_func_array([$array, 'add_rules'], $rules);
                            break;
                        case 'callbacks':
                            $callbacks = array_merge([$field], $value);
                            call_user_func_array([$array, 'add_callbacks'], $callbacks);
                            break;
                    }
                }
            }
    ...
    public function delete($id = null)
    {
        ...
        // Delete this object
        db::delete($this->table_name)->where($this->primary_key, '=', $id)->execute($this->db);
    ...
    {
        $result = db::select(['id' => $model->foreign_key($table)])->from($table)->where($this->foreign_key($table, $table), '=', $this->primary_key_value)->execute($this->db)->as_object();
